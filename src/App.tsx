...
import { UploadCloud, Image as ImageIcon, Loader2, BookOpen, AlertCircle, Network, ChevronRight, X, Target, Copy, Check, Download, Settings2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleCopyMarkdown = async () => {
    ...
    if (analysis.rawOcrText) {
      const ocrLines = formatMcqOptionsPerLine(analysis.rawOcrText).split('\n').join('\n> ');
      md += `> **📝 原始 OCR 文本：**\n> \n> ${ocrLines}\n\n`;
    }
  ...
  return (
    <div
      className={`min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-900 ${fontScaleClass}`}
    >
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm shadow-slate-200/60">
                    ...
                    {analysis.rawOcrText && (
                      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 border-l-[5px] border-l-slate-400 overflow-hidden">
                        <div className="px-5 pt-5 pb-2 border-b border-slate-100 bg-slate-50/60">
                          <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">① 识别文本</p>
                          ...
                          <p className="text-sm text-slate-500 mt-1">从图片中还原的文字，可与原图对照</p>
                        </div>
                        <div className="p-5">
                          <div className="max-h-52 overflow-y-auto rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3">
                            <p className="text-sm text-slate-700 font-mono whitespace-pre-wrap leading-[1.7]">
                              {formatMcqOptionsPerLine(analysis.rawOcrText)}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 border-l-[5px] border-l-emerald-500 overflow-hidden">
                    <div className="px-5 pt-5 pb-2 border-b border-emerald-100 bg-emerald-50/40">
                      ...
                      <p className="text-sm text-emerald-900/80 mt-1">
                        共 {analysis.knowledgePoints.length} 个核心考点 · {analysis.knowledgeTree.length} 组知识树节点
                      </p>
                    </div>
